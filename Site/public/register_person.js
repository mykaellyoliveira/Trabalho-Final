...
form.addEventListener('submit', (e) => {
    ...
    try {
        ...
        const person = new Person(name.value, email.value, telephone.value, gender.value === 'f' ? Gender.Female : Gender.Male, citys.value === 's' ? City.Sarandi : City.Maringa, plan.value === 'b' ? Plan.Básico : plan.value === 'm' ? Plan.Médio : Plan.Avançado, new Date(localdate));
        console.log(localdate);
        persons.push(person);
...
function showPersons() {
    if (localStorage.getItem('persons')) {
        ...
        for (const item of data) {
            persons.push(new Person(item.name, item.email, item.telephone, item.gender, item.city, item.plan, item.date));
        }
    }
}
